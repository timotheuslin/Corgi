;
; From http://www.autoitscript.com/forum/index.php?showtopic=29326&hl=CoProc&st=30
; Author:piccaso
...
_CoProcReceiver([$sFunction = ""]) Register/Unregister Reciver Function
...
Global $gs_CoProcReceiverFunction = ""
...
Func _CoProc($sFunction = Default, $vParameter = Default)
	...
	EndIf
	If @Compiled Then
		$iPid = Run(FileGetShortName(@AutoItExe), @WorkingDir, @SW_HIDE,  2 + 4)
	Else
		$iPid = Run(FileGetShortName(@AutoItExe) & ' "' & @ScriptFullPath & '"', @WorkingDir, @SW_HIDE, 2 + 4)
	EndIf
...
Local $iProcessId = WinGetProcess($hWnd)
...
;
;===============================================================================
Func _CoProcReceiver($sFunction = Default)
	Local $sHandlerFuction = "__CoProcReceiverHandler", $hWnd, $aTmp
	...
	$gs_CoProcReceiverFunction = $sFunction
	Return True
EndFunc ;==>_CoProcReceiver

Func __CoProcReceiverHandler($hWnd, $iMsg, $WParam, $LParam)
    If $iMsg = 0x4A Then; WM_COPYDATA
        Local $COPYDATA = DllStructCreate("ptr;dword;ptr", $LParam)
        Local $MyData = DllStructCreate("char[" & DllStructGetData($COPYDATA, 2) & "]", DllStructGetData($COPYDATA, 3))
        Local $msgParameter = DllStructGetData($MyData, 1)
    ...
    ElseIf $iMsg = 0x400 + 0x64 Then; WM_USER+0x64
        If UBound($gar_msgQueue)>1 Then
            ...
            Call($gs_CoProcReceiverFunction, $msgParameter)
            If @error And @Compiled = 0 Then MsgBox(16, "CoProc Error", "Unable to Call: " & $gs_CoProcReceiverFunction)
            Return 0
        EndIf
    EndIf
EndFunc ;==>__CoProcReceiverHandler
...
Func __CoProcDummy($vPar = Default)
	If Not IsKeyword($vPar) Then _CoProcReceiver($vPar)
